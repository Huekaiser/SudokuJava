...
public class TableroJuego9 extends SuperTableroJuego{
	private Integer[] id;
	private int quedan;
	protected ArrayList<JTextField> celdas;
	protected ControladorPartida cp;

	public TableroJuego9(final int posx, final int posy, final int width, final int height, final String dif, final boolean cargar, String nomP, Integer id2) throws ClassNotFoundException, IOException{
		...
		cct.borrarPartida(ControladorPresentacion.getUser(), nomP);
		...
		for ( int i3 = 0; i3 < mat.length; i3++) {
			for (int j = 0; j < mat.length; j++) {
				if(mat[i3][j]!=0){
					celdas.get(i3*9+j).setText(mat[i3][j].toString());
					if(cp.isPordefecto(i3,j)){
						celdas.get(i3*9+j).setEditable(false);
						celdas.get(i3*9+j).setFont(new Font("Verdana", Font.BOLD, 12));
					}
				}
				else{
					celdas.get(i3*9+j).setText("");
					celdas.get(i3*9+j).addFocusListener(new FocusAdapter() {
						@Override
						public void focusLost(FocusEvent e){
							...
							for (int i2  = 0; i2  < 9; i2 ++) {
								for (int j2 = 0; j2 < 9; j2++) {
									if(e.getSource()==celdas.get(i2*9+j2)){
										x=i2;
										y=j2;
									}
								}
							}
							if(valido(celdas.get(x*9+y).getText())){
								if(!cp.setNum(x, y, Integer.parseInt(celdas.get(x*9+y).getText()))){
									celdas.get(x*9+y).setText("");
									JOptionPane op= new JOptionPane();
									op.showMessageDialog(null, "Número no válido","Error", JOptionPane.ERROR_MESSAGE);
								}
								else{
									boolean acabado= true;
									Integer[][] matP= new Integer[9][9];
									...
									if(acabado){
										...
										try {
											cp.actualizarRecords(9, cp.getId(), cp.getDificultad(), m);
										} catch (ClassNotFoundException | IOException e1) {
											// TODO Auto-generated catch block
											e1.printStackTrace();
										}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(quedan>1){
				if(!cp.ayuda()) {
					...
					Integer[][] matA= new Integer[9][9];
					matA= cp.getMatrizValores();
					for (int i = 0; i < matA.length; i++) {
						for (int j = 0; j < matA.length; j++) {
							if(matA[i][j]!=0 && celdas.get(i*9+j).getText().equals("")){
								celdas.get(i*9+j).setText(matA[i][j].toString());
								celdas.get(i*9+j).setFont(new Font("Verdana", 0, 12));
								celdas.get(i*9+j).setForeground(Color.RED);
								celdas.get(i*9+j).setEditable(false);
